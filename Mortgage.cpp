...
bool moneyPaid(int64 pay) {
	int64 money = loan;
	forn(i, term*12 - 1) {
		money -= pay;
		if(money <= 0)	return true; 
		money += ceil(money*rate);
	}
	money -= pay;
	return money <= 0;
}

int64 solve(int64 low = 0, int64 high = (1ll << 60)) {
	int64 mid = (low + high)/2;
	bool money_paid = moneyPaid(mid);
	printf("low = %6lld, high = %6lld, mid = %6lld, moneyPaid = %s\n", low, high, mid, money_paid ? "Yes": "No");
