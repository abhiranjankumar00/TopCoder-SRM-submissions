...
{
    vector <vector<int> > v(guesses.size());
    vector <int> tmp;
    cout << "guesses.size() = " << guesses.size() << "\n";
    for(int i = 0; i < (int)guesses.size(); ++i) {
        if(valid(1ll*guesses[i] - answers[i]))
            v[i].push_back(guesses[i] - answers[i]);
        if(valid(1ll*guesses[i] + answers[i]))
            v[i].push_back(guesses[i] + answers[i]);
        //printf("%d, %d\n", guesses[i]-answers[i], guesses[i] + answers[i]);
    }
    tmp = v[0];
    vector <int> :: iterator it;
    for(int i = (int)1; i <= (int)v.size()-1; ++i) {
        it = set_intersection(v[i].begin(), v[i].end(), tmp.begin(), tmp.end(), tmp.begin());
