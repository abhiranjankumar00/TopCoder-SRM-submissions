...
{
public:
	bool predicate(int perWorker, int nWorker, vi & folders) {
		int currentLoad = 0;

		tr(it, folders) {
			if(currentLoad + *it > perWorker) {
				nWorker--;
				currentLoad = *it;
			}
			else
	...
	{
		int low = *max_element(all(folders)), high = accumulate(all(folders), 0);

		while(low < high) {
			int mid = low + (high - low) / 2;
	//		Pf("low = %d, high = %d, load = %d\n", low, high, mid);
			bool p = predicate(mid, workers, folders);
			if(p)
				high = mid;
			else
				low = mid + 1;
